...
public class TestLCA
{
    ...
    public static void main(String[] s)
    {
        ...
        try
        {
            for (; argp < s.length; argp++)
            {
                ...
                {
                    System.err.println("Could not recognise flag " +
                        s[argp]);
                    trouble = true;
                }
            }    
        ...
        timer.start();
        int tests = lcaCheck(lcaChecks, innerChecks, r, probChild,
            maxDepth);
        timer.stop();
        System.out.println("Time for " + tests + " inner tests is " +
            timer);
        System.out.println("All OK");
    }
    /** Create a tree for testing */
    private static TestNode buildTree(Random r, double probChild,
        int maxDepth, int depth, int[] num, TestNode parent)
    {
        if (depth > maxDepth)
            return null;
        int here = num[0];
        ...
        for (int i = 0; i < children; i++)
            nursery[i] = buildTree(r, probChild, maxDepth,
            depth + 1, num, tn);
        tn.setChildren(nursery);
        return tn;
    }
    /** Check cycle detect by introducing one, and checking that
     *  LCA detects it. Catches the resulting exception, so this
     *  routine throws iff LCA doesn't - unless something else goes
     *  wrong.
     */
    public static void cycleCheck(int cycleChecks, Random r,
        double probChild, int maxDepth)
    {
        ...
        for (int c = 0; c < cycleChecks;)
        {
            int[] num = new int[1];
            num[0] = 1;
            TestNode sample = buildTree(r, probChild, maxDepth, 0,
                                        num, null);
    ...
    /**
     * Check that the fast LCA algorithm works by comparing it with the
     * result of a slower one.
     */
    ...
    /** Set up an array of pointers into the tree, indexed by
     * depth first search number (counting from 1 - because that's
     * good for the LCA algorithm).
     */
    ...
    /** Simple Least Common Ancestor code to check - takes time
     * depending on distance to common ancestor.
     * @param a node to find LCA of
     * @param b node to find LCA of
     * @exception NullPointerException if two nodes not in same
     * tree, or other stupidity
     */
...
class TestNode implements TreeNode
{
    ...
    public int getDepth() {return ourDepth;}
    private int ourNum;
    ...
    public TestNode getTestChild(int x) {return children[x];} 
    public TreeNode getChild(int x) {return getTestChild(x);} 
    private TestNode parent;
    public TestNode getParent() {return parent;}
    public int getNum()
    {
        return ourNum;
    }
