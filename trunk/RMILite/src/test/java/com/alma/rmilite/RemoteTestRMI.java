		...
		final class RunnableServer implements Runnable {
			...
			public void run() {
		        ...
		        try {
		                ...
		                javaTask.setClassname(com.alma.rmilite.FakeServer.class.getName());
		                javaTask.setArgs("10010 --rmi");
		                Path path = new Path(project, new File(System.getProperty("user.dir") + "/target/test-classes/").getAbsolutePath());
		                path.add(new Path(project, new File(System.getProperty("user.dir") + "/target/classes/").getAbsolutePath()));
