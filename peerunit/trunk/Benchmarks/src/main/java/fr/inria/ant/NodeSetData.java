...
public class NodeSetData extends AbstractNodeData implements INodeSet {
    ...
    private void createHostSet() {
        ...
        if (stTo.countTokens() != 4) {
            throw new BuildException("L'ip to est mal formé");
        }
        ...
        if (Integer.parseInt(ipFrom4) > Integer.parseInt(ipTo4)) {
            throw new BuildException(
                    "La dernière partie de l'ip de l'attribut from est supérieure à celle de l'attribut to");
        }
    ...
    private void resolveIncludes() {
        ...
        // L'utilisateur a précisé d'autres noeuds
        if (getIncludes() != null) {
    ...
    /**
     * Permet de retirer les machines distantes définies dans l'attribut
     * excludes *
     */
