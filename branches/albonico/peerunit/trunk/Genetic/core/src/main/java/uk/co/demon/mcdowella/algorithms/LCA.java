...
/** The Lowest Common Ancestor algorithm of Schieber and Vishkin (sequential version only), roughly as described in "On finding lowest common ancestors: simplification and parallelization", by Baruch Schieber and Uzi Vishkin, SIAM J. Comput. Vol 17, No 6, December 1988
*/
...
public class LCA
{
    ...
    private int nextNum;
    /** detect all infinite loops due to cycles in graphs and perhaps
     *  some repeats due to acyclic graphs that have multiple pointers
     *  leading to the same node.
     */
    private CycleDetector cd;

    /** Create a structure used to answer Lowest Common Ancestor
     * queries, from a tree of objects implementing the TreeNode
     * interface, passed by a reference to its root.
     * Each query takes constant time, independent of the
     * size of the tree. Building this structure takes time linear
     * in the size of the tree.
     * @param n the root of the tree
     * @exception IllegalArgumentException if we find that we have
     * revisited a node on a depth first search. This will never
     * happen if our tree really
     * is a tree, because we will never revisit nodes in the same search.
     * It is sure to happen if our 'tree' is a
     * cyclic graph. Unintended revisits will happen in acyclic graphs, but
     * we may very well not detect them. We
     * could detect them for sure at small worst case constant cost,
     * but I can only see how to do it if we are allowed to mark TreeNodes,
     ...
     *  <p>
     *  We assign inLabels here. The inLabel of a node is the node number
     *  of that node or any of its descendants with the largest number of
     *  contiguous zeros ending in the lsb. The inLabels give rise to a
     *  binary tree structure where it is easy to work out LCAs.
     *  A node ending in e.g. abcd1000 has children
     *  abcd0100 and abcd1100, and descendants abcd0001..abcd1111. The
     *  level of each node in this tree depends on the position of its
     *  rightmost set bit. The ancestor at some level of any node can be
     *  found by setting the bit for that level and clearing all bits to
     *  its right.
     *  Each parent-child link in the real tree
     *  either corresponds to a parent-descendant path in the tree of
     *  inLabels, or to two nodes with the same inLabel. All sets of nodes
     *  with the same inLabel are single lines of parent-child links. Some
     *  parent-child links in the tree of inLabels are not
     *  ancestor-descendant links in the real tree, though.
     *  </p>
     *  @param n the TreeNode to work on
     *  @param depth the current recursion depth (starts depth = 0)
     *  @return the inLabel of TreeNode n.
     *  @exception IllegalStateException if the input tree is in fact
     *  a more general graph and we detect a repeated visit to a node
     *  in the tree because of this.
     */
    ...
    /** set up bitmap of true ancestors in each OurNode. These are the
     *  ancestors according to inLabel which are also ancestors according
     *  to the original links. Each ancestor is marked by the position of
     *  its lowest set bit. Count each node as its own ancestor, too
     */
    private void dfs2(OurNode target, int bits)
    {
        bits |= 1 << Log2.lowZeros(target.inLabel);
        target.trueAncestors = bits;
        for (target = target.firstChild; target != null;
             target = target.next)
        {
            dfs2(target, bits);
        }
    }
    /** Work out the lowest common ancestor of two nodes identified by
     *  their acceptDFSNum number, in constant time.
     * @param id1 the acceptDFSNum of a node requiring an ancestor
     * @param id2 the acceptDFSNum of a node requiring an ancestor
     * @return their lowest common ancestor
     * @exception IllegalArgumentException if rubbish numbers
     * @exception IllegalStateException if fails an internal check
     */
    public TreeNode LCA(int id1, int id2)
    {
        ...
        {  // On a path down the tree. Return the highest node
        ...
        if (ida.inLabel != targetLabel)
        { // The inLabel we seek is the highest true ancestor lower than
