...
#HADOOPTEST_JAR=./target/HadoopTest-1.0-SNAPSHOT-jar-with-dependencies.jar
HADOOPTEST_JAR=`cat hadoop.properties | grep ^hadooptest.classpath | cut -b 22-200`

if [ -d $HADOOP_HOME ] 
then
	echo $JAVA $HADOOP_OPTIONS -classpath $HADOOP_CLASSPATH:$PEERUNIT_JAR:$HADOOPTEST_JAR fr.inria.peerunit.TestRunner $HADOOP_TESTCASE 
	$JAVA $HADOOP_OPTIONS -classpath $HADOOP_CLASSPATH:$PEERUNIT_JAR:$HADOOPTEST_JAR fr.inria.peerunit.TestRunner $HADOOP_TESTCASE 
	#$JAVA $HADOOP_OPTIONS -classpath $HADOOP_CLASSPATH:$HADOOPTEST_JAR fr.inria.peerunit.TestRunner $HADOOP_TESTCASE 
else
