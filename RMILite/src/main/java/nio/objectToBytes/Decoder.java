...
public class Decoder {
	 ...
	 * further use
	 * @param buffer
	 *          the array of bytes to handle. Will be copied if some data needed
	 *          are missing, waiting for other arrays to be handled
	 * @param maxPos
	 *          last position in the index where bytes are meaningful. the array
	 ...
	 * @param startPos
	 *          the starting position of the sub array. Unused if <0. Method has
	 *          no effect if >
	 * @param buffer
	 *          .length
	 * @param endPos
	...
	public void decode( byte[] buffer, int startPos, int endPos,
		...
		case HEADER:
			for( ; receivedSize + bufferOffset < header.length
				...
				header[ receivedSize + bufferOffset ] = buffer[ bufferOffset + startPos ];
			}
			...
			if( receivedSize == header.length ) {
				...
				if( expectedSize > 0 ) {
					...
					// if no data is expected from the header, then it means we need
					// another header : data is already received as an empty array
					requireHeader();
				}
			}
		break;
