 ...
 * @author sunye
 */
public class TestPiEstimatorStrategy implements CoordinationStrategy {
    ...
    public void testcaseExecution() throws InterruptedException {
        ...
        LOG.entering("TestPiEstimatorStrategy", "testCaseExecution()");

        testers.execute("startNameNode");
        testers.execute("startDataNode");
        testers.execute("startJobTracker");
        testers.execute("startTaskTracker");

  //      if (testers.execute("jobSubmit")) {
            testers.execute("assertResult");
   //     }

        testers.execute("stopSlaveServices");
        testers.execute("stopMasterServices");
        testers.execute("ac");
