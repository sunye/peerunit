...
/** This class implements a version of the revised simplex method, similar to
that followed by Simplex, but using sparse matrix representations internally.
See alseo Simplex.
Both implement a basic version of the revised simplex
...
public class SimplexSparse
{
  ...
  /** set up the problem. c is the objective vector. A is the
    ...
    decrease them.
    */
  public SimplexSparse(double[] c, double[] A, double[] b, long seed, double forSmall)
  {
    small = forSmall;
    ...
    objective = c.clone();
    amat = new MatWrapper(b.length, c.length, A);
    araw = A.clone();
    bvec = b.clone();
    r = new Random(seed);
  }
  ...
  private final MatWrapper amat;
  /** Matrix A in raw form */
  private final double[] araw;
  ...
  /** Do up to the specified number of passes starting with the
      ...
      descrepancy[0]
      */
  public double searchFrom(int[] nonZeroX, int maxPasses, 
    double[] xValues,
    boolean[] foundBest, int[] numPassesDone, double[] discrepancy)
  {
    ...
    MatWrapper bmat = amat.selectColumns(nonZeroX);
    /*
    System.out.println("Select from " + Arrays.toString(nonZeroX) +
      " and " + amat);
    System.out.println("Initial bmat " + bmat);
    */
    MatWrapper binv = bmat.inverse();
    // System.out.println("Initial binv " + binv);
    // work out values implied for non-zero x
    double[] nzx = new double[nonZeroX.length];
    // Solution = B^-1  * constraint
    binv.preMult(bvec, nzx);
    // Will be kept at cost of current solution
    double cost = 0.0;
    double[] costs = new double[nonZeroX.length];
    // will use later on to work out whether to make variables non-zero
    double[] bulkChange = new double[objective.length];
	...
	*/
	double changeFromOtherVars = -bulkChange[i];
	...
	break;
      ...
      for (int i = 0; i < nzx.length; i++)
      {
        ti[i] = araw[i * objective.length + bestAt];
      }
      binv.preMult(ti, t);
	...
	System.out.println("Expect cost change " +
	  expectedChange + " to " + (cost + expectedChange));
        System.out.println("B is " + bmat);
        System.out.println("B^-1 is " + binv);
	System.out.print("Replace " + leaveAt + " by");
	for (int i = 0; i < nzx.length; i++)
	{
	  System.out.print(" " + araw[i * objective.length + bestAt]);
	}
	System.out.println();
      }
	...
	break;
      ...
      binv.updateInverse(leaveAt, t);
      // System.out.println("New binv is " + binv);
      // LU.print(nzx.length, binv);
      // update bmat
      bmat.updateMatrix(leaveAt, ti);
      // System.out.println("New B is " + bmat);
      if (EXPENSIVE_CHECKS)
      {
        // Check B^-1 B = I
	double[] unit = new double[nzx.length];
	for (int i = 0; i < nzx.length; i++)
	{
	  double[] row = binv.getRow(i);
	  bmat.postMult(row, unit);
	  for (int j = 0; j < nzx.length; j++)
	  {
	    ...
	    if (Math.abs(x) > small)
	    {
	      System.out.println("Bad inverse " + binv);
	      throw new IllegalStateException("B * B^-1 != I: " + x);
	    }
	  }
	}
      }
	...
	if (here < 0.0)
	{ // trouble!
	  System.err.println("Found -ve value");
	  discrepancy[0] = here;
	}
	costs[i] = objective[nonZeroX[i]];
	// newCost += here * objective[nonZeroX[i]];
      ...
      double newCost = MatWrapper.dot(nzx.length, 0, 1, nzx,
        0, 1, costs);
      /*
      Can get larger differences than this if B matrix is nearly
        singular
	...
	// If here, we have miscalculated effects => bug
	  ...
	  cost + " expected change " + expectedChange + " new cost " + newCost);
      }
      */
  ...
  public static void main(String[] s) throws IOException
  {
    ...
    SimplexSparse sx = new SimplexSparse(objective, a, b, 42, 1.0e-6);
